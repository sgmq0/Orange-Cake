...
void ChooseColor_float(float3 Highlight, float3 Middle, float3 Shadow, float Diffuse, float ThresholdLight, float ThresholdShadow, out float3 OUT)
{
    if (Diffuse < ThresholdShadow)
    {
        OUT = Shadow;
    }
    else if (Diffuse < ThresholdLight)
    {
        OUT = Middle;
    }
    else 
